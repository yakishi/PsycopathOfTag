...
public class TrapArrangeMent : NetworkBehaviour {
    ...
    public override void OnStartServer()
    {
        ...
        for (int i = 1; i < arrangePoint.Length + 1; i++) {
            arrangePoint[i - 1] = GameObject.Find("TrapArrangePoint" + i);
        }
    ...
    void Update()
    {
        ...
        if (timer.RemainTime <= 0 && !arrangeFlag) {
            var prefab = Resources.Load<GameObject>("Prefab/TrapObject" + randomValue);
